...
@plac.opt('feat', type=str, help="""Feature on which TIN needs to be calculated for a gene.
                        It should match with the 3rd column of the annotation file""")
@plac.opt('groupby', type=str, help="""Specify the attribute by which features need to be combined.
                        This should be present in the annotation file.""")
@plac.opt('strand', type=str, help="""Strand on which tin should be calculated.
                              Possible values are both, same, reverse""")
@plac.opt('libtype', type=str, help="""Specify if the library is paired or single. 
...
@plac.opt('count_cutoff', type=int, help="""Count cutoff to be used for checking overlaps. ie, minimum reads required to consider a gene as being expressed. 
                        Genes with count > count-cutoff and tin <tin-cutoff are checked for overlaps.
                        """)
def run(bams, ann="", feat='exon', groupby='gene_id', strand='both', libtype='single', read_len=100, bg=False, n=50,
    ...
    for idx, bam in enumerate(bams):
        ...
        if strand == "both":
            ...
            gtin_sense = tin.get_obs_tin(bam=pbam, bed=bed, strand="same", bgfile=bg_file, size=size, flag="gene_sense")
            gtin_antisense = tin.get_obs_tin(bam=pbam, bed=bed, strand="reverse", bgfile=bg_file, size=size,
                                             flag="gene_antisense")
